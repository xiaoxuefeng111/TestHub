  ...
  <div class="report-list">
    ...
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      ...
      <el-tab-pane label="套件报告" name="suite">
        ...
        <!-- 套件报告列表 -->
        <el-table v-loading="suiteLoading" :data="suiteReports" border stripe>
          <el-table-column
            prop="name"
            label="套件名称"
            min-width="140"
      ...
      <el-tab-pane label="用例报告" name="case">
        ...
        <!-- 用例报告列表 -->
        <el-table v-loading="caseLoading" :data="caseReports" border stripe>
          <el-table-column
            label="测试用例"
            min-width="140"
            show-overflow-tooltip
      ...
      <div v-if="selectedCase">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="执行设备">{{
            selectedCase.device_name || "-"
          }}</el-descriptions-item>
          <el-descriptions-item label="执行状态">
            <el-tag
              :type="
                getDisplayStatus(selectedCase.status, selectedCase.result).type
              "
            >
              {{
          ...
          <el-descriptions-item label="开始时间">{{
            formatDateTime(selectedCase.started_at)
          }}</el-descriptions-item>
          <el-descriptions-item label="结束时间">{{
            formatDateTime(selectedCase.finished_at)
          }}</el-descriptions-item>
          <el-descriptions-item label="执行耗时">{{
            formatDuration(selectedCase.duration)
          }}</el-descriptions-item>
          <el-descriptions-item label="步骤通过率">
            <span
              :style="{
                color: getPassRateColor(selectedCase.pass_rate),
                fontWeight: 'bold',
              }"
            >
        ...
        <div v-if="selectedCase.error_message" class="detail-section">
          <h4>错误信息</h4>
          <el-alert
            :title="selectedCase.error_message"
            type="error"
            show-icon
            :closable="false"
          />
