  ...
  <div class="page-container">
    ...
    >
      <div v-if="currentRecord" class="record-detail">
        ...
        <div v-if="currentRecord.app_device_name" class="detail-item">
          <span class="label">目标设备:</span>
          <span>
            {{ currentRecord.app_device_name }}
            <span v-if="currentRecord.app_device_serial">
              ({{ currentRecord.app_device_serial }})
            </span>
          </span>
        </div>
        <div v-if="currentRecord.app_package_name" class="detail-item">
          <span class="label">应用包:</span>
          <span>
            {{ currentRecord.app_package_name }}
            <span v-if="currentRecord.app_package_identifier">
              ({{ currentRecord.app_package_identifier }})
            </span>
          </span>
        </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button
            v-if="currentRecord && currentRecord.gif_path"
            type="primary"
            @click="viewGif(currentRecord)"
          >
            {{ $t("uiAutomation.ai.executionReport.viewGif") }}
          </el-button>
    ...
    <el-dialog
      ...
      append-to-body
    >
      <div v-if="currentGifUrl" class="gif-container">
        <img :src="currentGifUrl" alt="Execution GIF" class="gif-image" />
      </div>
    </el-dialog>
...
const getGifUrl = (gifPath) => {
  if (!gifPath) return "";
  const normalizedPath = gifPath.startsWith("media/")
    ? `/${gifPath}`
    : `/media/${gifPath}`;
...
const viewGif = (row) => {
  if (!row?.gif_path) return;
  currentGifUrl.value = getGifUrl(row.gif_path);
